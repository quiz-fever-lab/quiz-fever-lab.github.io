...
const quizTemplate = (quiz, questions, answers, currentIndex, onSelect, resetQuiz, onSubmit) => html`
    <section id="quiz">
        <header class="pad-large">
            <h1>
                ${quiz.title}: Question ${currentIndex + 1} / ${questions.length}
            </h1>
        ...
        <div class="pad-large alt-page">
            <article class="question">
                ...
                <nav class="q-control">
                    <span class="block">
                        ${answers.filter(a => a == undefined).length} questions remaining
                    </span>
                    ${currentIndex > 0 ? html`<a class="action" href="/quiz/${quiz.objectId}?question=${currentIndex}">
                        <i class="fas fa-arrow-left"></i>
                        Previous
                    </a>` : ''}
                    <a @click=${resetQuiz} class="action" href="javascript:void(0)">
                    ...
                    <div class="right-col">
                        ...
                        ${(answers.filter(a => a == undefined).length == 0 || 
                                ...
                                html`<a ${onSubmit} class="action" href="javascript:void(0)">
                            Submit answers
                        </a>` : ''}
                    </div>
                </nav>
...
const questionTemplate = (question, index, isCurrent) => html`
    <div data-index="question-${index}" style=${styleMap({display: isCurrent ? '' : 'display:none'})}>
